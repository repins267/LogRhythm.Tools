...
Function Update-ExaCorrelationRule {
    <#
    .SYNOPSIS
        Update an Exabeam correlation rule.
    .DESCRIPTION
        Updates an existing correlation rule in the Exabeam environment.
        Requires the rule ID and the fields to update.
    .PARAMETER Id
        The correlation rule ID (UUID) to update.
    .PARAMETER Name
    ...
    .OUTPUTS
        PSCustomObject representing the updated correlation rule when PassThru is specified.
    .EXAMPLE
        PS C:\> Update-ExaCorrelationRule -Id "cbfa855a-..." -Severity critical -PassThru
        ---
        Updates the severity of the specified correlation rule.
    .NOTES
        Exabeam-API
    .LINK
    ...
    Process {
        ...
        if ($PSCmdlet.ShouldProcess($Id, "Update Exabeam correlation rule")) {
            # Send Request
