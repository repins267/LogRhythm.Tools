...
Function Get-LrLsvTemplateItems {
    <#
    .SYNOPSIS
        Retrieve items for a Log Source Virtualization template from LogRhythm.
    .DESCRIPTION
        Get-LrLsvTemplateItems returns all LSV template items for the specified
        template, including regular expressions and configuration details.
    .PARAMETER Id
        The LSV Template ID to retrieve items for.
    ...
    .OUTPUTS
        PSCustomObject representing LogRhythm LSV Template Items.
    .EXAMPLE
        PS C:\> Get-LrLsvTemplateItems -Id 5
        ---
        Returns all items for LSV Template ID 5.
    .NOTES
        LogRhythm-API
    .LINK
    ...
    Process {
        ...
        # Request URL
        $RequestUrl = $BaseUrl + "/lr-admin-api/lsvtemplates/" + $Id + "/lsvtemplateitems/" + $QueryString
