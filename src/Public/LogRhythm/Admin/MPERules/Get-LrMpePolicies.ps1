...
Function Get-LrMpePolicies {
    <#
    .SYNOPSIS
        Retrieve a list of MPE Policies from the LogRhythm.
    .DESCRIPTION
        Get-LrMpePolicies returns a list of accepted Log Sources, including details.
    .PARAMETER Id
    ...
    Process {
        ...
        if ($msgSourceTypeId) {
            $_id = $msgSourceTypeId
            $QueryParams.Add("messageSourceTypeId", $_id)
        }
        ...
        # Request URL
        $RequestUrl = $BaseUrl + "/lr-admin-api/mpepolicies/" + $QueryString
