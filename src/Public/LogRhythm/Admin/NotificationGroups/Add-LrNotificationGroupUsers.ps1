...
Function Add-LrNotificationGroupUsers {
    <#
    .SYNOPSIS
        Add persons to a notification group in the LogRhythm Admin API.
    .DESCRIPTION
        Add-LrNotificationGroupUsers assigns one or more person IDs to an
        existing notification group.
    .PARAMETER Id
        The notification group ID to add persons to.
    .PARAMETER PersonIds
        An array of person IDs to add to the notification group.
    .PARAMETER PassThru
    ...
    .EXAMPLE
        PS C:\> Add-LrNotificationGroupUsers -Id 1 -PersonIds @(1, 2, 3) -PassThru
    .NOTES
        LogRhythm-API
    .LINK
    ...
    Process {
        ...
        # Ensure single-element arrays are serialized as arrays
        if ($PersonIds.Count -eq 1) { $Body = "[$Body]" }
