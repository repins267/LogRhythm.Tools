...
Function Get-LrUserPermissions {
    <#
    .SYNOPSIS
        Retrieve user permission records from the LogRhythm Admin API.
    .DESCRIPTION
        Get-LrUserPermissions returns user records from the Admin API users
        endpoint with automatic pagination support.
    ...
    .OUTPUTS
        PSCustomObject representing LogRhythm user permission records.
    .EXAMPLE
        PS C:\> Get-LrUserPermissions
    .NOTES
        LogRhythm-API
    .LINK
    ...
    Process {
        ...
        $RequestUrl = $BaseUrl + "/lr-admin-api/permissions/" + $QueryString
