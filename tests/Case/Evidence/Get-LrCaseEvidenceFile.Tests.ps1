Describe "LogRhythm.Tools: Get-LrCaseEvidenceFile" -Tag 'Unit' {
    BeforeAll {
        # Mock dependencies
    ...
    Context "Input validation" {
        It "Should have CmdletBinding attribute" {
            (Get-Command Get-LrCaseEvidenceFile).CmdletBinding | Should -BeTrue
        }

        It "Should have mandatory Id parameter" {
            (Get-Command Get-LrCaseEvidenceFile).Parameters['Id'].Attributes.Where({ $_ -is [System.Management.Automation.ParameterAttribute] }).Mandatory | Should -BeTrue
        }

        It "Should have mandatory EvidenceId parameter" {
            (Get-Command Get-LrCaseEvidenceFile).Parameters['EvidenceId'].Attributes.Where({ $_ -is [System.Management.Automation.ParameterAttribute] }).Mandatory | Should -BeTrue
        }
    }
