Describe "LogRhythm.Tools: Get-LrPlaybookAttachments" -Tag 'Unit' {
    BeforeAll {
        # Mock dependencies
    ...
    Context "Input validation" {
        It "Should have CmdletBinding attribute" {
            (Get-Command Get-LrPlaybookAttachments).CmdletBinding | Should -BeTrue
        }

        It "Should have mandatory Id parameter" {
            (Get-Command Get-LrPlaybookAttachments).Parameters['Id'].Attributes.Where({ $_ -is [System.Management.Automation.ParameterAttribute] }).Mandatory | Should -BeTrue
        }
    }
