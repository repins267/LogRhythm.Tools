InModuleScope 'LogRhythm.Tools' {
    Describe "LogRhythm.Tools: Update-LrBeat" -Tag 'Unit' {
        BeforeAll {
            # Mock dependencies
            Mock Invoke-RestAPIMethod { return [PSCustomObject]@{ id = 1; name = "UpdatedBeat" } }
            Mock Enable-TrustAllCertsPolicy { }
        }
    
        Context "Input validation" {
            It "Should have CmdletBinding attribute" {
                (Get-Command Update-LrBeat).CmdletBinding | Should -BeTrue
            }
    
            It "Should have mandatory Id parameter" {
                (Get-Command Update-LrBeat).Parameters['Id'].Attributes.Where({ $_ -is [System.Management.Automation.ParameterAttribute] }).Mandatory | Should -BeTrue
            }
    
            It "-PassThru parameter should exist" {
                (Get-Command Update-LrBeat).Parameters.Keys | Should -Contain 'PassThru'
            }
        }
