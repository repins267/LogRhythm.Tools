Describe "LogRhythm.Tools: Update-LrBeatHeartbeat" -Tag 'Unit' {
    BeforeAll {
        # Mock dependencies
    ...
    Context "Input validation" {
        It "Should have CmdletBinding attribute" {
            (Get-Command Update-LrBeatHeartbeat).CmdletBinding | Should -BeTrue
        }

        It "Should have mandatory Ids parameter" {
            (Get-Command Update-LrBeatHeartbeat).Parameters['Ids'].Attributes.Where({ $_ -is [System.Management.Automation.ParameterAttribute] }).Mandatory | Should -BeTrue
        }

        It "-PassThru parameter should exist" {
            (Get-Command Update-LrBeatHeartbeat).Parameters.Keys | Should -Contain 'PassThru'
        }
    }
