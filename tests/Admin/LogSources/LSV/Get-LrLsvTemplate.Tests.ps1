InModuleScope 'LogRhythm.Tools' {
    Describe "LogRhythm.Tools: Get-LrLsvTemplate" -Tag 'Unit' {
        BeforeAll {
            # Mock dependencies
        ...
        Context "Input validation" {
            It "Should have CmdletBinding attribute" {
                (Get-Command Get-LrLsvTemplate).CmdletBinding | Should -BeTrue
            }
    
            It "Should have mandatory Id parameter" {
                (Get-Command Get-LrLsvTemplate).Parameters['Id'].Attributes.Where({ $_ -is [System.Management.Automation.ParameterAttribute] }).Mandatory | Should -BeTrue
            }
        }
