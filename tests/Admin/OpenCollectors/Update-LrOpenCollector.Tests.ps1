InModuleScope 'LogRhythm.Tools' {
    Describe "LogRhythm.Tools: Update-LrOpenCollector" -Tag 'Unit' {
        BeforeAll {
            # Mock dependencies
        ...
        Context "Input validation" {
            It "Should have CmdletBinding attribute" {
                (Get-Command Update-LrOpenCollector).CmdletBinding | Should -BeTrue
            }
    
            It "Should have mandatory Id parameter" {
                (Get-Command Update-LrOpenCollector).Parameters['Id'].Attributes.Mandatory | Should -Contain $true
            }
    
            It "-PassThru parameter should exist" {
                (Get-Command Update-LrOpenCollector).Parameters.Keys | Should -Contain 'PassThru'
            }
        }
