InModuleScope 'LogRhythm.Tools' {
    Describe "LogRhythm.Tools: New-LrOpenCollector" -Tag 'Unit' {
        BeforeAll {
            # Mock dependencies
        ...
        Context "Input validation" {
            It "Should have CmdletBinding attribute" {
                (Get-Command New-LrOpenCollector).CmdletBinding | Should -BeTrue
            }
    
            It "Should have mandatory Name parameter" {
                (Get-Command New-LrOpenCollector).Parameters['Name'].Attributes.Mandatory | Should -Contain $true
            }
    
            It "-PassThru parameter should exist" {
                (Get-Command New-LrOpenCollector).Parameters.Keys | Should -Contain 'PassThru'
            }
        }
