InModuleScope 'LogRhythm.Tools' {
    Describe "LogRhythm.Tools: Get-LrOpenCollectorBeats" -Tag 'Unit' {
        BeforeAll {
            # Mock dependencies
        ...
        Context "Input validation" {
            It "Should have CmdletBinding attribute" {
                (Get-Command Get-LrOpenCollectorBeats).CmdletBinding | Should -BeTrue
            }
    
            It "Should have mandatory Id parameter" {
                (Get-Command Get-LrOpenCollectorBeats).Parameters['Id'].Attributes.Mandatory | Should -Contain $true
            }
        }
