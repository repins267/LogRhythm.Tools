Describe "LogRhythm.Tools: Get-LrOpenCollector" -Tag 'Unit' {
    BeforeAll {
        # Mock dependencies
    ...
    Context "Input validation" {
        It "Should have CmdletBinding attribute" {
            (Get-Command Get-LrOpenCollector).CmdletBinding | Should -BeTrue
        }

        It "Should have mandatory Id parameter" {
            (Get-Command Get-LrOpenCollector).Parameters['Id'].Attributes.Mandatory | Should -Contain $true
        }
    }
