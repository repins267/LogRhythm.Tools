InModuleScope 'LogRhythm.Tools' {
    Describe "LogRhythm.Tools: Update-LrOpenCollectorHeartbeat" -Tag 'Unit' {
        BeforeAll {
            # Mock dependencies
        ...
        Context "Input validation" {
            It "Should have CmdletBinding attribute" {
                (Get-Command Update-LrOpenCollectorHeartbeat).CmdletBinding | Should -BeTrue
            }
    
            It "Should have mandatory Id parameter" {
                (Get-Command Update-LrOpenCollectorHeartbeat).Parameters['Id'].Attributes.Mandatory | Should -Contain $true
            }
    
            It "-PassThru parameter should exist" {
                (Get-Command Update-LrOpenCollectorHeartbeat).Parameters.Keys | Should -Contain 'PassThru'
            }
        }
