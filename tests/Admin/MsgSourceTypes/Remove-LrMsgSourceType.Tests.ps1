Describe "LogRhythm.Tools: Remove-LrMsgSourceType" -Tag 'Unit' {
    BeforeAll {
        # Mock dependencies
    ...
    Context "Input validation" {
        It "Should have CmdletBinding attribute" {
            (Get-Command Remove-LrMsgSourceType).CmdletBinding | Should -BeTrue
        }

        It "Should have mandatory Id parameter" {
            (Get-Command Remove-LrMsgSourceType).Parameters['Id'].Attributes.Where({ $_ -is [System.Management.Automation.ParameterAttribute] }).Mandatory | Should -BeTrue
        }

        It "-PassThru parameter should exist" {
            (Get-Command Remove-LrMsgSourceType).Parameters.Keys | Should -Contain 'PassThru'
        }
    }
