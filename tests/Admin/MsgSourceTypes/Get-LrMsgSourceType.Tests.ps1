Describe "LogRhythm.Tools: Get-LrMsgSourceType" -Tag 'Unit' {
    BeforeAll {
        # Mock dependencies
        Mock Invoke-RestAPIMethod { return [PSCustomObject]@{ id = 1; name = "TestType" } }
        Mock Enable-TrustAllCertsPolicy { }
    }

    Context "Input validation" {
        It "Should have CmdletBinding attribute" {
            (Get-Command Get-LrMsgSourceType).CmdletBinding | Should -BeTrue
        }

        It "Should have mandatory Id parameter" {
            (Get-Command Get-LrMsgSourceType).Parameters['Id'].Attributes.Where({ $_ -is [System.Management.Automation.ParameterAttribute] }).Mandatory | Should -BeTrue
        }
    }
