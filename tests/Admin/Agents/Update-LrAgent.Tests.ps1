Describe "LogRhythm.Tools: Update-LrAgent" -Tag 'Unit' {
    BeforeAll {
        # Mock dependencies
    ...
    Context "Input validation" {
        It "Should have CmdletBinding attribute" {
            (Get-Command Update-LrAgent).CmdletBinding | Should -BeTrue
        }

        It "Should have mandatory Id parameter" {
            (Get-Command Update-LrAgent).Parameters['Id'].Attributes.Mandatory | Should -Contain $true
        }

        It "-PassThru parameter should exist" {
            (Get-Command Update-LrAgent).Parameters.Keys | Should -Contain 'PassThru'
        }
    }
