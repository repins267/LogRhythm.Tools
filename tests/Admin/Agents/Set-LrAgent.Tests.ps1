Describe "LogRhythm.Tools: Set-LrAgent" -Tag 'Unit' {
    BeforeAll {
        # Mock dependencies
    ...
    Context "Input validation" {
        It "Should have CmdletBinding attribute" {
            (Get-Command Set-LrAgent).CmdletBinding | Should -BeTrue
        }

        It "Should have mandatory Id parameter" {
            (Get-Command Set-LrAgent).Parameters['Id'].Attributes.Mandatory | Should -Contain $true
        }

        It "-PassThru parameter should exist" {
            (Get-Command Set-LrAgent).Parameters.Keys | Should -Contain 'PassThru'
        }
    }
