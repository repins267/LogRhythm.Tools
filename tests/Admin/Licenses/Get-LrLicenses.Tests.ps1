Describe "LogRhythm.Tools: Get-LrLicenses" -Tag 'Unit' {
    BeforeAll {
        # Mock dependencies
        Mock Invoke-RestAPIMethod { return @([PSCustomObject]@{ id = 1; name = "License1" }) }
        Mock Enable-TrustAllCertsPolicy { }
    }

    Context "Input validation" {
        It "Should have CmdletBinding attribute" {
            (Get-Command Get-LrLicenses).CmdletBinding | Should -BeTrue
        }
    }
