Describe "LogRhythm.Tools: Get-LrLicenseEntitlements" -Tag 'Unit' {
    BeforeAll {
        # Mock dependencies
        Mock Invoke-RestAPIMethod { return @([PSCustomObject]@{ id = 1; entitlement = "Feature1" }) }
        Mock Enable-TrustAllCertsPolicy { }
    }

    Context "Input validation" {
        It "Should have CmdletBinding attribute" {
            (Get-Command Get-LrLicenseEntitlements).CmdletBinding | Should -BeTrue
        }
    }
