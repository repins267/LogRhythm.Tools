InModuleScope 'LogRhythm.Tools' {
    Describe "LogRhythm.Tools: Get-LrUserProfileLogSources" -Tag 'Unit' {
        BeforeAll {
            # Mock dependencies
            Mock Invoke-RestAPIMethod { return [PSCustomObject]@{ id = 1; name = "TestLogSource" } }
            Mock Enable-TrustAllCertsPolicy { }
        }
    
        Context "Input validation" {
            It "Should have CmdletBinding attribute" {
                (Get-Command Get-LrUserProfileLogSources).CmdletBinding | Should -BeTrue
            }
    
            It "Should have mandatory Id parameter" {
                (Get-Command Get-LrUserProfileLogSources).Parameters['Id'].Attributes.Mandatory | Should -Contain $true
            }
        }
