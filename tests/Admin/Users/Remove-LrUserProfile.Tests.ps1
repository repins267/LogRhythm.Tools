Describe "LogRhythm.Tools: Remove-LrUserProfile" -Tag 'Unit' {
    BeforeAll {
        # Mock dependencies
        Mock Invoke-RestAPIMethod { return [PSCustomObject]@{ statusCode = 200 } }
        Mock Enable-TrustAllCertsPolicy { }
    }

    Context "Input validation" {
        It "Should have CmdletBinding attribute" {
            (Get-Command Remove-LrUserProfile).CmdletBinding | Should -BeTrue
        }

        It "Should have mandatory Id parameter" {
            (Get-Command Remove-LrUserProfile).Parameters['Id'].Attributes.Mandatory | Should -Contain $true
        }
    }
    ...
    Context "Mutating cmdlet requirements" {
        It "-PassThru parameter should exist" {
            (Get-Command Remove-LrUserProfile).Parameters.Keys | Should -Contain 'PassThru'
        }
    }
}
