Describe "LogRhythm.Tools: Get-LrUserProfilePrivileges" -Tag 'Unit' {
    BeforeAll {
        # Mock dependencies
        Mock Invoke-RestAPIMethod { return [PSCustomObject]@{ id = 1; name = "TestPrivilege" } }
        Mock Enable-TrustAllCertsPolicy { }
    }

    Context "Input validation" {
        It "Should have CmdletBinding attribute" {
            (Get-Command Get-LrUserProfilePrivileges).CmdletBinding | Should -BeTrue
        }

        It "Should have mandatory Id parameter" {
            (Get-Command Get-LrUserProfilePrivileges).Parameters['Id'].Attributes.Mandatory | Should -Contain $true
        }
    }
