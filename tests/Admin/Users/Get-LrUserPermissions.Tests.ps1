InModuleScope 'LogRhythm.Tools' {
    Describe "LogRhythm.Tools: Get-LrUserPermissions" -Tag 'Unit' {
        BeforeAll {
            # Mock dependencies
            Mock Invoke-RestAPIMethod { return [PSCustomObject]@{ id = 1; name = "TestPermission" } }
            Mock Enable-TrustAllCertsPolicy { }
        }
    
        Context "Input validation" {
            It "Should have CmdletBinding attribute" {
                (Get-Command Get-LrUserPermissions).CmdletBinding | Should -BeTrue
            }
        }
