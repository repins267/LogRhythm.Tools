Describe "LogRhythm.Tools: Invoke-LrTrueIdentityConflictMerger" -Tag 'Unit' {
    BeforeAll {
        # Mock dependencies
        Mock Invoke-RestAPIMethod { return [PSCustomObject]@{ id = 1; status = "merged" } }
        Mock Enable-TrustAllCertsPolicy { }
    }

    Context "Input validation" {
        It "Should have CmdletBinding attribute" {
            (Get-Command Invoke-LrTrueIdentityConflictMerger).CmdletBinding | Should -BeTrue
        }

        It "-PassThru parameter should exist" {
            (Get-Command Invoke-LrTrueIdentityConflictMerger).Parameters.Keys | Should -Contain 'PassThru'
        }
    }
