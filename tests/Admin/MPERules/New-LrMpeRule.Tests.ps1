InModuleScope 'LogRhythm.Tools' {
    Describe "LogRhythm.Tools: New-LrMpeRule" -Tag 'Unit' {
        BeforeAll {
            # Mock dependencies
        ...
        Context "Input validation" {
            It "Should have CmdletBinding attribute" {
                (Get-Command New-LrMpeRule).CmdletBinding | Should -BeTrue
            }
    
            It "Should have mandatory Name parameter" {
                (Get-Command New-LrMpeRule).Parameters['Name'].Attributes.Where({ $_ -is [System.Management.Automation.ParameterAttribute] }).Mandatory | Should -BeTrue
            }
    
            It "-PassThru parameter should exist" {
                (Get-Command New-LrMpeRule).Parameters.Keys | Should -Contain 'PassThru'
            }
        }
